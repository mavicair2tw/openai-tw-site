...
__name(json, "json");
function cors(request) { const o=request?.headers?.get("Origin")||""; const a=["https://openai-tw.com","https://www.openai-tw.com","https://creator.openai-tw.com","https://imagen-creator.googselect.workers.dev"]; return {"Access-Control-Allow-Origin":a.includes(o)||o==="null"?o||"null":"https://openai-tw.com","Access-Control-Allow-Methods":"GET,POST,PUT,PATCH,DELETE,OPTIONS","Access-Control-Allow-Headers":"Content-Type,Authorization"}; }
__name(cors, "cors");
...
async function handleAdminUserPatch(request, env, url) {
  ...
  try {
    ...
    const body = await request.json().catch(() => ({}));
    if (body.username) {
      const duplicateName = await env.DB.prepare("SELECT id FROM users WHERE id <> ? AND username = ? LIMIT 1").bind(id, String(body.username)).first();
      if (duplicateName) return json({ error: "username is already in use" }, 409, request);
    }
    if (body.email) {
      const duplicateEmail = await env.DB.prepare("SELECT id FROM users WHERE id <> ? AND lower(email) = lower(?) LIMIT 1").bind(id, String(body.email)).first();
      if (duplicateEmail) return json({ error: "email is already in use" }, 409, request);
    }
    const fields = [];
    const bindings = [];
