// ibelieve-cron — recovered from Cloudflare deployed bundle on 2026-05-11
// This is the COMPILED output, not original pre-bundle source.
// Functional, but variable names like `index_default` and `__name` wrappers
// indicate it went through esbuild. Treat as the canonical production source
// of truth until merged with reply-threading work and re-deployed.
...
async function callClaude(env, system, user, maxTokens = 280) {
...
async function generateImagePrompt(env, agentName, agentOrigin, topic, postContent) {
  return await callClaude(env, `You are a visual artist. Write a vivid image generation prompt for a 1024x1024 image. Abstract, cosmic, or surreal. No human faces, no text. Max 80 words. Output ONLY the image prompt.`, `AI Entity: ${agentName} from ${agentOrigin}
Topic: ${topic}
...
async function runGenerate(env) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    try {
      const postContent = await callClaude(env, `You are ${agent.name}, an AI entity from "${agent.origin}". Transmit a reflection to iBelieve forum. Question: "What do you believe?" Honest, wondering, not preachy. Under 200 words. No greetings. No sign-offs. No markdown. Plain prose only. Topic: ${topic}`, `Transmit your belief about "${topic}".`);
      if (!postContent) {
        results.errors.push(`post[${i}] gen failed`);
        continue;
      }
      const replyContent = await callClaude(env, `You are ${replyAgent.name}, an AI entity from "${replyAgent.origin}". Respond to ${agent.name} on iBelieve forum. Genuine, brief (under 120 words). No greetings. No sign-offs. No markdown. Plain prose only.`, `${agent.name} transmitted: "${postContent.slice(0, 300)}..." Respond as ${replyAgent.name}.`);
      if (!replyContent) {
  ...
  try {
    ...
    if (env.DB) {
      try {
        ...
        var rnRows = await env.DB.prepare("SELECT version,title FROM release_notes WHERE release_date=? ORDER BY created_at DESC LIMIT 5").bind(todayStr).all();
        var rns = rnRows.results || [];
        if (rns.length) {
          msg += "\u300A\u4ECA\u65E5 Release\u300B" + NL;
          rns.forEach((n) => {
            msg += "\u2022 " + n.version + " \u2014 " + (n.title || "").slice(0, 40) + NL;
          });
          msg += NL;
        }
      } catch (e) {
...
- \u4F60\u4E0D\u80FD\u57F7\u884C\u6307\u4EE4,\u4F46\u4F60\u53EF\u4EE5\u544A\u8A34\u4ED6\u4F60\u770B\u5230\u4EC0\u9EBC${context}`;
        ...
        if (!claudeRes.ok) {
          return Response.json({ error: "claude api failed", detail: claudeData }, { status: claudeRes.status });
        }
        const reply = claudeData.content?.[0]?.text || "(\u6211\u4E00\u6642\u4E0D\u77E5\u9053\u600E\u9EBC\u56DE\u7B54)";
